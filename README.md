...
文档语言：简体中文 ｜ [English](README.en.md)

## 支持版本

- **已验证支持：禅道 18.6**，使用禅道 REST API v1。
- **兼容范围：禅道 18.x 的标准 REST API v1 部分**。不同小版本、版本类型、权限和自定义字段可能存在差异，除 18.6 外不承诺所有功能完全兼容。
- 需求、任务和 Bug 的必填项由禅道实例配置决定；Bug `deadline`、`consumed` 等字段只有在目标实例实际提供对应表单字段时才会生效。
...
本项目目前处于早期开发阶段，不排除后续出现破坏兼容性的修改。项目以 MIT 协议开源，请在 MIT 协议允许的范围内合理、合法使用。

如果这个项目对你有帮助，欢迎在 GitHub 点个 Star；如果你愿意参与开发，也欢迎提交 PR 或通过 Issue 与我交流。

本项目欢迎 LinuxDo 社区用户反馈和共建，欢迎访问 [LinuxDo 社区](https://linux.do/)。

禅道及其相关商标归其权利人所有。本项目是第三方 Claude Code 插件和 MCP 工具，与禅道官方不存在隶属、授权或官方合作关系，请注意区分。
